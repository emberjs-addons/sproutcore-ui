...
/** @scope UI.LayoutManager.prototype */{
  ...
  _layoutProperties: ['top', 'right', 'bottom', 'left', 'width', 'height'],
  _layout: null,
  ...
  _applyLayout: function(buffer,layout) {
    ...
    var layoutProperties = this._layoutProperties;
        len = layoutProperties.length;

    for (var i = 0; i < len; i++) {
      var prop = layoutProperties[i];

      if (layout[prop] !== undefined && layout[prop] !== null) {

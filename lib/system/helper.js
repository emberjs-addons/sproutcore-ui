 ...
 */
Handlebars.registerHelper('controller',function(path, options){
  
  // Normalize path
  var viewController = (path.indexOf('.') >= 0)? SC.getPath(path) : UI[path];
  if (!viewController.isInstance) { viewController = viewController.create(); }

  set(viewController, 'rootNode', options.data.view);

  viewController.appendViews();
});

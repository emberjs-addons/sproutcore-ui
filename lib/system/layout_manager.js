...
/** @scope UI.LayoutManager.prototype */{
  ...
  destroyAnchor: function(view) {
    ...
    for (var anchor in anchors) {
      if (anchors.hasOwnProperty(anchor) && anchor.view === view) {
        console.log('destroying',andhor);
        break;
      }
    }
  },
  ...
  _reflowContentView: function() {
    ...
    var element = get(contentView,'element');
    ...
    if (element) {
      ...
      var buffer = get(contentView,'buffer');

      for (var prop in layout) {
        //buffer.style(prop,layout[prop]); 
        console.log('setting',prop,'to',layout[prop]);
      }
    }
  },
  ...
  inspect: function () {
    ...
    for (var anchor in anchors) {
      ...
      if (anchors.hasOwnProperty(anchor)) {
        var constraintName = this._propertyMetadata[anchor].constraint;
        var constraint = anchors[anchor].constraint;

        console.log('Anchor: ',anchor,constraintName,constraint);
      }
    }

    var contentView = this._contentView;
    if (contentView) {
      console.log('Content view: ',contentView.view.toString(),'before: ',contentView.before,'after: ',contentView.after);
    }
  }
});
